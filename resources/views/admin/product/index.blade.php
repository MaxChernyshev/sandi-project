            ...
            <div class="min-w-[1170px]">
                ...
                @foreach($products as $product)
                    <div class="bg-white dark:bg-boxdark">
                        <!-- table row item -->
                        <div class="grid grid-cols-4 border-t border-[#EEEEEE] px-5 py-4 dark:border-strokedark lg:px-7.5 2xl:px-11">
                            ...
                            <div class="flex flex-row w-full px-4 py-2 text-sm hover:bg-whiter hover:text-primary dark:hover:bg-meta-4">
                                <p class="text-[#637381] dark:text-bodydark">{{ $product->category }}</p>
                            </div>

                            {{-- TODO insert LiveWire for Image with temporary URL--}}

                            <div class="flex flex-row w-full px-4 py-2 text-sm hover:bg-whiter hover:text-primary dark:hover:bg-meta-4">
                                <img class="object-none h-16 w-16" src="{{ $product->image ?$product->image->image_path : '/storage/no-photos.png' }}">
                            </div>
